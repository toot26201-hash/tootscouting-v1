...
# --- دالة إضافة اللوجو (Watermark) ثابتة للكل ---
def add_logo(ax):
    try:
        # بيقرأ ملف الصورة اللي إنت رفعته
        img = Image.open('image_deac96.png')
        ax.imshow(img, extent=[48, 72, 28, 52], alpha=0.15, zorder=0)
    except:
        pass

st.title("⚽ TootScouting | Integrated Tactical Analysis")
...
if uploaded_file is not None:
    ...
    # ---------------------------------------------------------
    # TAB 1: التحليل الفردي (ألوان موحدة + لوجو)
    # ---------------------------------------------------------
    with tab1:
    ...
    # ---------------------------------------------------------
    # TAB 2: التحليل الجماعي (نفس الألوان + لوجو + خيارات تكتيكية)
    # ---------------------------------------------------------
    with tab2:
        st.subheader(f"Team Tactical Master: {selected_team}")
        team_options = st.multiselect("Tactical Layers", 
            ["Normal Passes", "Crosses", "Through Balls", "Goals/Shots", "Area 14 Hub", "Box Entries", "Touches inside Box"], 
        ...
        add_logo(ax_t) # اللوجو في الجماعي
        ...
        for i, row in team_df.iterrows():
            ...
            # 1. التمريرات والكروسات (نفس السيستم اللوني)
    ...
    st.info("👋 ارفع ملف الـ CSV وهتلاقي كل الخصائص (الألوان + اللوجو) موجودة في التحليل الفردي والجماعي!")
