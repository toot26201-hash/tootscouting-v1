...
if uploaded_file:
    ...
    # تصنيف دقيق للأنواع داخل البوكس وخارجه
    def classify(row):
        val = str(row['Action']).lower()
        x, y = row['x_scaled'], row['y_scaled']
        in_box = (102 <= x <= 120) and (18 <= y <= 62)
        
        # تصنيف خاص للداخل البوكس
        if in_box:
            if 'shot' in val: return "Shot (Box)"
            if 'pass' in val: return "Pass (Box)"
            if 'dribble' in val: return "Dribble (Box)"
            return "Touch (Box)"
    ...
    # لوحة الألوان
    colors = {
        "Shot (Box)": "#FF0000", "Pass (Box)": "#00FF00", 
        "Dribble (Box)": "#FF00FF", "Touch (Box)": "#FFFFFF",
        "Cross": "#FF4500", "Corner": "#00BFFF", 
        "Shot": "#8B0000", "Zone 14": "#FFD700", "Other": "#808080"
    }
    ...
    with col1:
        ...
        for act in selected_actions:
            ...
            pitch.scatter(subset['x_scaled'], subset['y_scaled'], color=color, s=150, ax=ax1, label=act)
