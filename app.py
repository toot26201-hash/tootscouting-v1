...
if uploaded_file is not None:
    ...
    # --- الدليل الشامل (المرجع النهائي لكل الرموز) ---
    def get_full_legend():
        return [
            ...
            mlines.Line2D([], [], color='blue', marker='x', linestyle='None', markersize=10, label='Tackle (Blue X)'),
            ...
            mlines.Line2D([], [], color='red', marker='x', linestyle='None', label='Foul (Red X)'),
    ...
    # ---------------------------------------------------------
    # TAB 2: التحليل الجماعي (خيارات مستقلة)
    # ---------------------------------------------------------
    with tab2:
        st.subheader(f"Team Tactical Master: {selected_team}")
        col1, col2 = st.columns(2)
        with col1:
            p_opt = st.multiselect("Passing Layers", ["Normal Passes", "Crosses", "Through Balls", "Corners", "Free Kicks"], default=["Normal Passes", "Crosses"])
        with col2:
            d_opt = st.multiselect("Defense & Attack Layers", ["Tackles", "Ground Duels", "Aerial Duels", "Clearances", "Fouls", "Counterpress", "Goals"], default=["Tackles", "Goals"])
        ...
        for i, row in team_df.iterrows():
            ...
            # التمريرات
            if "Normal Passes" in p_opt and "pass" in act and all(k not in tag for k in ["cross", "through", "corner", "free kick"]):
            ...
            if "Corners" in p_opt and "corner" in tag:
            ...
            # الدفاع
            if "Tackles" in d_opt and "tackle" in act:
