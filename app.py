...
st.title("⚽ TootScouting | Precision Tactical System")
...
if uploaded_file is not None:
    ...
    # ---------------------------------------------------------
    # TAB 2: التحليل الجماعي (فصل الخيارات)
    # ---------------------------------------------------------
    with tab2:
        st.subheader(f"Team Tactical Master: {selected_team}")
        
        # فصل كل نوع تمريرة وأكشن في اختيار لوحده
        col1, col2 = st.columns(2)
        with col1:
            pass_opts = st.multiselect("Passing Options", ["Normal Passes", "Crosses", "Through Balls", "Corners", "Free Kicks"], default=["Normal Passes", "Crosses"])
        with col2:
            def_opts = st.multiselect("Defensive & Attack Options", ["Aerial Duels", "Ground Duels", "Fouls", "Counterpress", "Goals"], default=["Ground Duels", "Goals"])
        ...
        for i, row in team_df.iterrows():
            ...
            # منطق التمريرات المفصلة
            if "Normal Passes" in pass_opts and "pass" in act and all(k not in tag for k in ["cross", "through", "corner", "free kick"]):
            ...
            if "Crosses" in pass_opts and "cross" in tag:
            ...
            if "Through Balls" in pass_opts and "through" in tag:
                pitch_t.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='#FF69B4', ax=ax_t)
            if "Corners" in pass_opts and "corner" in tag:
                pitch_t.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='orange' if is_success else 'red', linestyle='solid' if is_success else 'dashed', ax=ax_t)
            if "Free Kicks" in pass_opts and "free kick" in tag:
                pitch_t.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='#40E0D0', ax=ax_t)

            # منطق الدفاع والهجوم المفصل
            if "Aerial Duels" in def_opts and "aerial" in act:
                pitch_t.scatter(row.x_scaled, row.y_scaled, marker='^', s=150, color='#2ecc71' if is_success else 'red', ax=ax_t)
            if "Ground Duels" in def_opts and ("tackle" in act or "duel" in act) and "aerial" not in act:
                pitch_t.scatter(row.x_scaled, row.y_scaled, marker='s', s=120, color='#2ecc71' if is_success else 'red', ax=ax_t)
            if "Fouls" in def_opts and "foul" in act:
                pitch_t.scatter(row.x_scaled, row.y_scaled, marker='x', s=180, color='red', ax=ax_t)
            if "Counterpress" in def_opts and ("counter" in act or "press" in act):
                ax_t.text(row.x_scaled, row.y_scaled, '#', color='black', fontsize=15, ha='center', va='center')
            if "Goals" in def_opts and "goal" in tag:
    ...
    st.info("👋 ارفع ملف الـ CSV.. كل نوع تمريرة وأكشن بقى متاح لوحده!")
