...
if uploaded_file is not None:
    ...
    def get_full_legend():
        return [
            ...
            mlines.Line2D([], [], color='#ef4444', marker='x', label='Foul (Red X)', linestyle='None', markersize=10, markeredgewidth=2),
            mlines.Line2D([], [], color='#E2E8F0', marker='o', label='Counterpress (#)', linestyle='None', markersize=8),
            mlines.Line2D([], [], color='#FFCC00', marker='*', label='Goal Confirmed ⚽', linestyle='None', markersize=15)
        ]
        ...
        for i, row in dataframe.iterrows():
            ...
            if 'goal' in act or 'goal' in tag:
                ...
                if draw_mode and (specific_type is None or specific_type == "passes" or specific_type == "all") and "Goals" in layers:
                    pitch_obj.scatter(row.x_scaled, row.y_scaled, marker='*', s=750, color='#FFCC00', edgecolors='black', ax=ax, zorder=6)
            ...
            if 'pass' in act:
                ...
                if 'key' in tag or 'key pass' in tag:
                    ...
                    if draw_mode and (specific_type is None or specific_type == "passes" or specific_type == "all") and "Key Passes" in layers:
                        pitch_obj.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=3, color='#FFCC00', ax=ax, zorder=4)
                    ...
                    if draw_mode and (specific_type is None or specific_type == "passes" or specific_type == "all"):
                        pitch_obj.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='#FF00FF', ax=ax, zorder=4)
                    matrix["through_balls"] += 1
                    action_captured = True
                elif "Normal Passes" in layers and not any(k in tag for k in ['cross', 'through', 'corner']):
                    if draw_mode and (specific_type is None or specific_type == "passes" or specific_type == "all"):
                        pitch_obj.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='#00FF66' if is_success else '#FF3333', ax=ax, alpha=0.7, zorder=3)
                    action_captured = True
                    ...
                    if draw_mode and (specific_type is None or specific_type == "crosses" or specific_type == "all"):
                        pitch_obj.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='#FFCC00' if is_success else '#FF6666', ax=ax, zorder=4)
                    action_captured = True
                elif 'cross' in tag and "Crosses" in layers:
                    if draw_mode and (specific_type is None or specific_type == "crosses" or specific_type == "all"):
                        pitch_obj.arrows(row.x_scaled, row.y_scaled, row.x_end_scaled, row.y_end_scaled, width=2, color='#00CCFF' if is_success else '#FF6666', linestyle='solid' if is_success else 'dashed', ax=ax, zorder=4)
                    matrix["crosses"] += 1
                ...
                if 'clearance' in tag or 'clearance' in act:
                    if "Clearances" in layers:
                        if draw_mode and (specific_type is None or specific_type == "defense" or specific_type == "all"):
                            pitch_obj.scatter(row.x_scaled, row.y_scaled, marker='d', s=200, color='#CC99FF', ax=ax, zorder=5)
                    matrix["clearances"] += 1
                else:
                    if "Tackles" in layers:
                        if draw_mode and (specific_type is None or specific_type == "defense" or specific_type == "all"):
                            pitch_obj.scatter(row.x_scaled, row.y_scaled, marker='x', s=240, color='#00CCFF', linewidth=3, ax=ax, zorder=5)
                    matrix["tackles"] += 1
                ...
                if "Counterpress" in layers:
                    if draw_mode and (specific_type is None or specific_type == "defense" or specific_type == "all"):
                        ax.text(row.x_scaled, row.y_scaled, '#', color='#E2E8F0', fontsize=22, fontweight='bold', ha='center', va='center', zorder=5)
